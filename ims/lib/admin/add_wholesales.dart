...
class AddWholesalesPage extends StatefulWidget {
  const AddWholesalesPage({Key? key}) : super(key: key);

  @override
  AddWholesalesPageState createState() => AddWholesalesPageState();
}

class AddWholesalesPageState extends State<AddWholesalesPage> {
  ...
  Future<void> submitSalesData() async {
    try {
      ...
      var response = await http.post(
        Uri.parse('http://10.0.2.2:8000/api/add_wholesales/'),
        headers: {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Add Wholesales'),
      ),
      body: Form(
        key: _formKey,
        child: ListView(
